...
def api_listening_patterns():
    ...
    if 'artist_name' in df.columns:
        ...
        if not artists_df.empty:
            top_artists_series = artists_df.groupby('artist_name').size().sort_values(ascending=False).head(5)
    ...
    if 'track_name' in df.columns and 'artist_name' in df.columns:
        ...
        if not tracks_df.empty:
            top_tracks_df = tracks_df.groupby(['track_name', 'artist_name']).size().sort_values(ascending=False).head(5)
